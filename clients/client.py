...
class Client:
    ...
    def updateLastOnline(self):
        self.patch(self.apiUrl + "/api/device/" + str(self.clientId) + "/update")

    def getDevice(self):
        response = self.get(self.apiUrl + "/api/device/" + self.clientId + "/");
        #self.patch(self.apiUrl + "/api/device/" + self.clientId + "/jobs/update?status=done")
        print(response)
